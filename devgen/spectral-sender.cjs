/* ============================================================================
 * spectral-sender.cjs — Node for Max side of NAM_A2_Spectral (Contract 6). v4.
 * ---------------------------------------------------------------------------
 * v4 (2026-07-21 ROOT CAUSE): the repo root package.json declares
 * "type": "module" (ESM), so any .js under the repo tree gets `require`
 * REMOVED by Node — require('max-api') threw, the guard swallowed it, and the
 * sender ran handler-less ("Unhandled Message" floods; the entire v1 mystery).
 * The .cjs extension forces CommonJS in EVERY location, ending the class.
 * ----------------------------------------------------------------------------
...
if (maxApi) {
  ...
  maxApi.addHandler(maxApi.MESSAGE_TYPES.ALL, (...raw) => {
    let a = raw;
    // OBSERVED ON THE RIG 2026-07-21: the ALL handler's real signature is
    // (handledFlag, selector, ...args) — a leading BOOLEAN says whether a
    // named handler already took the message. Strip it, then the selector.
    if (typeof a[0] === 'boolean') a = a.slice(1);
  ...
  maxApi.post('spectral-sender: up v3.1 (SPC1 -> ' + HOST + ':' + PORT + ')');
}
